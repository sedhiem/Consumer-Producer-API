...
{
public:
  bool operator()(const cs::Entry* entry1, const cs::Entry* entry2) const
  {
...
public:
   ...
   */
  const Data* find(const Interest& interest);
...
private:
   ...
   *  and a flag indicating if the entry was newly created (True) or refreshed
   * (False) }
   */
  std::pair<cs::Entry*, bool> insertToSkipList(const Data& data,
                                               bool isUnsolicited = false);

  /** \brief Removes a specific CS Entry from all layers of a skip list
   *  \return{ returns True if CS Entry was succesfully removed and False if CS
   * Entry was not found}
   */
  bool eraseFromSkipList(cs::Entry* entry);
   ...
   *  startingPoint can be equal to Interest Name only when the item is in the
   * begin() position.
   *
   *  Iterates toward greater Names, terminates when CS entry falls out of
   * Interest prefix.
   *  When childSelector = leftmost, returns first CS entry that satisfies other
   * selectors.
   *  When childSelector = rightmost, it goes till the end, and returns CS entry
   * that satisfies
   *  other selectors. Returned CS entry is the leftmost child of the rightmost
   * child.
   *  \return{ the best match, if any; otherwise 0 }
   */
  const Data* selectChild(const Interest& interest,
                          SkipListLayer::iterator startingPoint) const;

  /** \brief checks if Content Store entry satisfies Interest selectors
   * (MinSuffixComponents,
   *  MaxSuffixComponents, Implicit Digest, MustBeFresh)
   *  \return{ true if satisfies all selectors; false otherwise }
   */
  bool doesComplyWithSelectors(const Interest& interest, cs::Entry* entry,
                               bool doesInterestContainDigest) const;

  /** \brief interprets minSuffixComponent and name lengths to understand if
   * Interest contains
   *  implicit digest of the data
   *  \return{ True if Interest name contains digest; False otherwise }
   */
  bool recognizeInterestWithDigest(const Interest& interest,
...
private:
  ...
  size_t
    m_nMaxPackets; // user defined maximum size of the Content Store in packets
